...
beforeAll(async () => {
  await signInWithEmailAndPassword(auth, "manager@example.com", "password");
});
...
describe("Manager", () => {
  ...
  it("can update service/conf", async () => {
    await expect(updateDoc(refConf, data1)).resolves.toBeUndefined();
  });
  ...
  it("can read service/auth", async () => {
    await expect(getDoc(refAuth)).rejects.toThrow();
  });
  it("can update service/auth", async () => {
    await expect(updateDoc(refAuth, data1)).rejects.toThrow();
  });
  ...
  it("can create groups/*", async () => {
    await expect(addDoc(refGroups, data1)).resolves.toHaveProperty("id");
  });
  it("can not update groups/group01", async () => {
    await expect(updateDoc(refGroup01, data1)).resolves.toBeUndefined();
  });
  ...
  it("can create users/*", async () => {
    await expect(addDoc(refUsers, data1)).resolves.toHaveProperty("id");
  });
  ...
  it("can update users/user02", async () => {
    await expect(updateDoc(refUser02, data1)).resolves.toBeUndefined();
  });
  ...
  it("can update posts/post01", async () => {
    await expect(updateDoc(refPost01, data1)).resolves.toBeUndefined();
  });
