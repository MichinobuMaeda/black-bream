...
describe("Guest", () => {
  ...
  it("can read groups/*", async () => {
    await expect(getDocs(refGroups)).rejects.toThrow();
  });
  ...
  it("can read users/*", async () => {
    await expect(getDocs(refUsers)).rejects.toThrow();
  });
  it("can not create users/*", async () => {
    await expect(addDoc(refUsers, data1)).rejects.toThrow();
  });
  it("can not update users/me", async () => {
    await expect(updateDoc(me, data1)).rejects.toThrow();
  });
  ...
  it("can read posts/*", async () => {
    await expect(getDocs(refPosts)).rejects.toThrow();
  });
