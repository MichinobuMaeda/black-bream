...
const me = refOperator;

beforeAll(async () => {
  await signInWithEmailAndPassword(auth, "operator@example.com", "password");
});
...
describe("Manager", () => {
  ...
  it("can not update service/conf", async () => {
  ...
  it("can create posts/*", async () => {
    await expect(addDoc(refPosts, data1)).resolves.toHaveProperty("id");
  });
